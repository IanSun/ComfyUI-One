...
export function create_before_register_node_definition_hook(
	...
	return (_, definition) => {
		...
		transform_input_specifications_type(
			...
			(specification) => {
				return Boolean(
					"COMFY_AUTOGROW_V3" === specification.type
					&& (specification as unknown as AutogrowSpecification).template["@one"],
				);
			},
			(specification) => {
	...
	for (const i of input) {
		...
		if (slots.every(({ name }) => n !== name)) {
			node.addInput(
				...
				i.type,
				{
					localized_name: resolve_autogrow_input_localized_name(node, name, i.name, index, locale),
	...
	node.onConnectionsChange = serialize(
		...
		(type, _, connected, link, slot) => {
			...
			for (const index of indexes) {
				remove_autogrow_group(node, app, name, index, map);
			}
...
function remove_autogrow_group(node: AutogrowNode, app: ComfyApp, name: string, index: number, map: Map<string, number>): void {
...
function resolve_autogrow_input_localized_name(node: AutogrowNode, name: string, key: string, index: number, locale: string): string {
