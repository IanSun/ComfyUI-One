...
class OneMaskScaleByShorterEdgeInputs(TypedDict):
	device: str
	mask: Tensor
	method: ScaleMethod
	size: int

class OneMaskScaleByShorterEdge(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneMaskScaleByShorterEdge",
			category = "One/Mask",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneMaskScaleByShorterEdgeInputs]) -> io.NodeOutput:
		mask = kwargs["mask"]

		return OneMaskScaleBy.execute(
			mask = mask,
			method = kwargs["method"],
			multiple = kwargs["size"] / min(mask.shape[1:3]),
