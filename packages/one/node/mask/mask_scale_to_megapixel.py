...
class OneMaskScaleToMegapixelInputs(TypedDict):
	device: str
	mask: Tensor
	megapixel: int
	method: ScaleMethod

class OneMaskScaleToMegapixel(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneMaskScaleToMegapixel",
			category = "One/Mask",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneMaskScaleToMegapixelInputs]) -> io.NodeOutput:
		mask = kwargs["mask"]
