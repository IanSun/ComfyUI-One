...
class OneMaskScaleUpByLongerEdgeInputs(TypedDict):
	device: str
	mask: Tensor
	method: ScaleMethod
	size: int

class OneMaskScaleUpByLongerEdge(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneMaskScaleUpByLongerEdge",
			category = "One/Mask",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneMaskScaleUpByLongerEdgeInputs]) -> io.NodeOutput:
		mask = kwargs["mask"]

		return OneMaskScaleBy.execute(
			mask = mask,
			method = kwargs["method"],
			multiple = max(1.0, kwargs["size"] / max(mask.shape[1:3])),
