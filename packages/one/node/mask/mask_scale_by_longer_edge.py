...
class OneMaskScaleByLongerEdgeInputs(TypedDict):
	device: str
	mask: Tensor
	method: ScaleMethod
	size: int

class OneMaskScaleByLongerEdge(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneMaskScaleByLongerEdge",
			category = "One/Mask",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneMaskScaleByLongerEdgeInputs]) -> io.NodeOutput:
		mask = kwargs["mask"]

		return OneMaskScaleBy.execute(
			mask = mask,
			method = kwargs["method"],
			multiple = kwargs["size"] / max(mask.shape[1:3]),
