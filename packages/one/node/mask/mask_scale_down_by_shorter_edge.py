...
class OneMaskScaleDownByShorterEdgeInputs(TypedDict):
	device: str
	mask: Tensor
	method: ScaleMethod
	size: int

class OneMaskScaleDownByShorterEdge(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneMaskScaleDownByShorterEdge",
			category = "One/Mask",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneMaskScaleDownByShorterEdgeInputs]) -> io.NodeOutput:
		mask = kwargs["mask"]

		return OneMaskScaleBy.execute(
			mask = mask,
			method = kwargs["method"],
			multiple = min(1.0, kwargs["size"] / min(mask.shape[1:3])),
