...
class OneImageScaleByLongerEdgeInputs(TypedDict):
	device: str
	image: Tensor
	method: ScaleMethod
	size: int

class OneImageScaleByLongerEdge(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneImageScaleByLongerEdge",
			category = "One/Image",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneImageScaleByLongerEdgeInputs]) -> io.NodeOutput:
		image = kwargs["image"]

		return OneImageScaleBy.execute(
			image = image,
			method = kwargs["method"],
			multiple = kwargs["size"] / max(image.shape[1:3]),
