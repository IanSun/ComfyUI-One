...
class OneImageScaleUpToMegapixelInputs(TypedDict):
	device: str
	image: Tensor
	megapixel: int
	method: ScaleMethod

class OneImageScaleUpToMegapixel(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneImageScaleUpToMegapixel",
			category = "One/Image",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneImageScaleUpToMegapixelInputs]) -> io.NodeOutput:
		image = kwargs["image"]
		...
		return OneImageScaleBy.execute(
			...
			multiple = max(1.0, kwargs["megapixel"] * 1e6 / (shape[1] * shape[2])),
