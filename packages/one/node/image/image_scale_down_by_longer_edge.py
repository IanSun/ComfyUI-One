...
class OneImageScaleDownByLongerEdgeInputs(TypedDict):
	device: str
	image: Tensor
	method: ScaleMethod
	size: int

class OneImageScaleDownByLongerEdge(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneImageScaleDownByLongerEdge",
			category = "One/Image",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneImageScaleDownByLongerEdgeInputs]) -> io.NodeOutput:
		image = kwargs["image"]

		return OneImageScaleBy.execute(
			image = image,
			method = kwargs["method"],
			multiple = min(1.0, kwargs["size"] / max(image.shape[1:3])),
