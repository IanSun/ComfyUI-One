...
class OneImageScaleByShorterEdgeInputs(TypedDict):
	device: str
	image: Tensor
	method: ScaleMethod
	size: int

class OneImageScaleByShorterEdge(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneImageScaleByShorterEdge",
			category = "One/Image",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneImageScaleByShorterEdgeInputs]) -> io.NodeOutput:
		image = kwargs["image"]

		return OneImageScaleBy.execute(
			image = image,
			method = kwargs["method"],
			multiple = kwargs["size"] / min(image.shape[1:3]),
