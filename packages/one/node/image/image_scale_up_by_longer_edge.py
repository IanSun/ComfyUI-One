...
class OneImageScaleUpByLongerEdgeInputs(TypedDict):
	device: str
	image: Tensor
	method: ScaleMethod
	size: int

class OneImageScaleUpByLongerEdge(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneImageScaleUpByLongerEdge",
			category = "One/Image",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneImageScaleUpByLongerEdgeInputs]) -> io.NodeOutput:
		image = kwargs["image"]

		return OneImageScaleBy.execute(
			image = image,
			method = kwargs["method"],
			multiple = max(1.0, kwargs["size"] / max(image.shape[1:3])),
