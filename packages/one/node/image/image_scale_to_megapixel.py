...
class OneImageScaleToMegapixelInputs(TypedDict):
	device: str
	image: Tensor
	megapixel: int
	method: ScaleMethod

class OneImageScaleToMegapixel(io.ComfyNode):
	@classmethod
	def define_schema(cls) -> io.Schema:
		return io.Schema(
			node_id = "OneImageScaleToMegapixel",
			category = "One/Image",
			inputs = [
	...
	@classmethod
	def execute(cls, **kwargs: Unpack[OneImageScaleToMegapixelInputs]) -> io.NodeOutput:
		image = kwargs["image"]
		...
		return OneImageScaleBy.execute(
			...
			multiple = kwargs["megapixel"] * 1e6 / (shape[1] * shape[2]),
