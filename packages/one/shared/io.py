...
class OneAutogrow(io.Autogrow):
	...
	class TemplatePrefix(io.Autogrow.TemplatePrefix):
		class OneOverrides(TypedDict):
			max: int | None
			min: int

		overrides: OneOverrides | None = None
...
class OneScaleMethod(io.Combo):
	class Input(io.Combo.Input):
		def __init__(
			...
			tooltip: str | None = None,
			lazy: bool | None = None,
			default: ScaleMethod = ScaleMethod.NearestExact,
			socketless: bool | None = None,
			extra_dict: dict[Any, Any] | None = None,
			raw_link: bool | None = None,
			advanced: bool | None = None,
